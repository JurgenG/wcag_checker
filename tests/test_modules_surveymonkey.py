"""Tests for the SurveyMonkey module."""
...
@pytest.fixture
def m():
    return module_by_id("surveymonkey")


def test_identity(m) -> None:
    assert m.module_id == "surveymonkey"
    assert m.module_name == "SurveyMonkey"
    assert m.legal_jurisdiction == "US"
    assert m.vendor and m.data_residency and m.sovereignty_notes


@pytest.mark.parametrize("host", ["widget.surveymonkey.com", "surveymonkey.com"])
...
def test_case_insensitive(m) -> None:
    event = make_request(host="widget.surveymonkey.com".upper(), url="https://X/x")
    assert m.matches(event) is True


def test_rejects_lookalike(m) -> None:
    event = make_request(host="surveymonkey.com.evil.example", url="https://surveymonkey.com.evil.example/x")
    assert m.matches(event) is False


def test_unknown_param_is_other(m) -> None:
    event = make_request(host="widget.surveymonkey.com", url="https://widget.surveymonkey.com/x?weird=1")
    hit = m.parse(event)
